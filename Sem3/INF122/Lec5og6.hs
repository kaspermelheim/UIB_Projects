...
--Tar inn streng, gjør heile strengen til lowercase
lower s = [toLower b | b <- s ]

--Passord må ha minst 5 små, 3 store bokstaver og 2 siffer
lud ps = [length [x | x <- ps, isLower x],
          length [x | x <- ps, isUpper x],
          length [x | x <- ps, isDigit x] ]

--sjekker alle element i lista fra lud
sjekk ps l u d = and [x<=y | (x,y) <- zip [l,u,d] (lud ps)]

rever xs = rev xs []
rev [] akk = akk
rev (x:xs) akk = rev xs (x:akk)
